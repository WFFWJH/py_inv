...
for k=max(slip_model(:,1)):-1:1
    this_seg=slip_model(slip_model(:,1)==k,:);
    ...
    for n=1:max(this_seg(:,3))
        this_layer=this_seg(this_seg(:,3)==n,:);
        if n==1
            xstart=x_offset;ystart=0;
        else
            xstart=x_offset;ystart=ystart-ddl(n-1);
        end
    ...
    [v,~,idx]=unique(this_seg(:,7));
    seg_l = unique(v.*accumarray(idx,1));
    x_offset = x_offset +seg_l(1)+5000;
    ...
    % hc = colorbar('southoutside');
    hc = colorbar;
ticks = hc.Ticks;hc.Ticks = unique([ticks, slip_max]);
    title(hc,'slip (m)');
    % if cmax < 5
    %     caxis([-cmax cmax]);
    % else
